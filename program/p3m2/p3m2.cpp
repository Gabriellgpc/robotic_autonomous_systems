...
{
    static_cast<Simulation_p3m2 *>(X)->_receiver_routine();
};
void _plotter_func(void *X)
{
    static_cast<Simulation_p3m2 *>(X)->_plotter_routine();
};

Simulation_p3m2::Simulation_p3m2() : thr_plotter(),
                                     thr_receiver(),
                                     mtx(),
                                     client(nullptr),
                                     time_to_stop(0.0),
...
Simulation_p3m2::~Simulation_p3m2()
{
    this->stop_simulation();
}

void Simulation_p3m2::stop_simulation()
{
    this->to_stop = true;
...
void Simulation_p3m2::_receiver_routine()
{
    double tik, tok;
...
void Simulation_p3m2::_plotter_routine()
{
    ...
    gp_work << "set title 'Grade de Ocupação'\n";
...
void Simulation_p3m2::_readDatas()
{
...
void Simulation_p3m2::_sensor_to_world_frame(MyDatas &datas)
{
    for (int is = 0; is < N_SENSORS; is++)
    {
        if (datas.sensors[is].zt() < 0)
