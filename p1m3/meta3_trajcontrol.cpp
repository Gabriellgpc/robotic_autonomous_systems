...
int main(){
    ...
    {   
        ...
        pioneer_velocity = b0RemoteApi::readFloat(client.simxGetObjectVelocity(pioneer, client.simxServiceCall()),1);
        double config[] = {pioneer_pos[0], pioneer_pos[1], pioneer_ori[2]};
        bool done = trajcontrol.step(config, pioneer_velocity, v, w);
        // converte saidas do controlador para velocidades dos motores
        pioneer_model(v, w, w_r, w_l);
        // pioneer_model(0.01, 0.0, w_r, w_l);
